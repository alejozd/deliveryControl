...
const Quoter = ({ onSave, quotationData, user }) => {
  ...
  const [selectedBodega, setSelectedBodega] = useState(""); // -1 para múltiples bodegas
    ...
    const fetchBodegas = async () => {
      try {
        ...
        if (response.status === 200) {
          const formattedBodegas = [
            { label: "Múltiples Bodegas", value: "-1" },
            ...response.data.result.map((bodega) => ({
              label: bodega.nombrebodega,
              value: String(bodega.codigobodega),
            })),
          ];
    ...
    try {
      ...
      if (
        ...
        toast.current.show({
          severity: "error",
          summary: "Error",
          detail: "Debe seleccionar una bodega antes de buscar productos",
          life: 3000,
        });
        return; // Detiene la ejecución si no hay bodega seleccionada
      }
      setLoading(true);
      ...
      if (response.data.status === 200) {
        const adaptedProducts = response.data.data.map((product) => {
          const bodegasAdaptadas = product.bodegas.map((b) => ({
            codigobodega: b.s_bodega,
            nombrebodega: b.s_nombre,
            existencia: b.s_existencia,
            cantidad: 0, // Se inicia en 0 para después modificar en la expansión
          }));

          return {
            ...product,
            nombreproductos: product.nombreproducto,
            bodegas: bodegasAdaptadas, // ahora sí, adaptadas
            existencia:
              selectedBodega === -1
                ? bodegasAdaptadas.reduce((acc, b) => acc + b.existencia, 0)
                : bodegasAdaptadas.find(
                    (b) => b.codigobodega === selectedBodega
                  )?.existencia || 0,
            nombrebodega:
              selectedBodega === -1
                ? "Múltiples Bodegas"
  ...
  const addProductsToDataTable = async (selectedProductsToAdd) => {
    setLoading(true);
    // console.log("selectedProductsToAdd", selectedProductsToAdd);
    try {
      // Paso 1: Crear mapa de productos actuales en la grilla
      const currentProductMap = new Map();
      selectedProducts.forEach((p) => {
        const key = `${p.codigo}-${p.subcodigo}-${p.bodega}`;
        currentProductMap.set(key, p);
      });

      // Paso 2: Identificar productos que necesitan actualización o inserción
      const productsToInsert = [];
      const productsToUpdate = [];
      const productsForPriceQuery = new Map();

      selectedProductsToAdd.forEach((product) => {
        const key = `${product.codigo}-${product.subcodigo}-${product.bodega}`;
        const existing = currentProductMap.get(key);
        ...
        if (existing) {
          ...
          const priceKey = `${product.codigo}-${product.subcodigo}`;
          if (
            !selectedProducts.some(
              (p) =>
                p.codigo === product.codigo && p.subcodigo === product.subcodigo
            )
          ) {
            productsForPriceQuery.set(priceKey, product);
          }
        }
      });

      // Paso 3: Consultar precios solo para nuevos productos (por código-subcódigo)
      const fetchedPrices = await Promise.all(
        Array.from(productsForPriceQuery.values()).map(async (product) => {
          try {
            ...
            if (response.data.status === 200) {
              return {
                key: `${product.codigo}-${product.subcodigo}`,
                precio: response.data.data[0].precio,
                tarifaiva: response.data.data[0].tarifaiva,
              };
            } else {
              handleError(response.data.error);
              return null;
      ...
      // Paso 4: Armar mapa de precios
      const priceMap = new Map();
      fetchedPrices.forEach((entry) => {
        if (entry) {
          priceMap.set(entry.key, {
            precio: entry.precio,
            tarifaiva: entry.tarifaiva,
          });
        }
      });

      // Paso 5: Armar productos finales para insertar
      const finalInsertProducts = productsToInsert.map((product, index) => {
        const priceKey = `${product.codigo}-${product.subcodigo}`;
        const priceInfo =
        ...
        const vrdescuento = (priceInfo.precio * cantidad * pordescuento) / 100;
        const total = priceInfo.precio * cantidad - vrdescuento;

        return {
          ...product,
          precio: priceInfo.precio,
          tarifaiva: priceInfo.tarifaiva,
          cantidad,
          pordescuento,
          vrdescuento,
          total,
          orden: selectedProducts.length + index + 1,
        };
      });

      const validInsertProducts = finalInsertProducts.filter((p) => p !== null);

      // Paso 6: Actualizar y agregar productos
      setSelectedProducts((prev) => {
        const updated = prev.map((p) => {
          const update = productsToUpdate.find(
            (u) =>
              u.codigo === p.codigo &&
              u.subcodigo === p.subcodigo &&
              u.bodega === p.bodega
          );
          return update ? { ...p, cantidad: update.cantidad } : p;
        });

        return [...updated, ...validInsertProducts];
      });
    } finally {
      setLoading(false);
    }
  ...
  const renderDiscountValue = (rowData) => {
    return (
      <InputNumber
        ...
        value={rowData.vrdescuento === 0 ? "" : rowData.vrdescuento} // Si es 0, muestra campo vacío
        mode="currency"
        currency="COP"
        onChange={(e) => handleDiscountValueChange(e, rowData)}
        onBlur={(e) => {
          if (e.target.value === "") {
            // Solo asigna 0 al perder el foco
            handleDiscountValueChange({ value: 0 }, rowData);
          }
        }}
        minFractionDigits={2}
        locale="es-CO"
        style={{ width: "8rem" }}
        inputStyle={{ minWidth: "6rem" }}
      />
    );
  };
  ...
  const handleBodegaChange = (e) => {
    ...
    if (selectedProducts.length > 0) {
      toast.current.show({
        ...
        summary: "Advertencia",
        detail:
          "No puede cambiar la bodega cuando hay productos en la cotización",
        life: 3000,
      });
      return; // Evita el cambio si hay productos
    }
  ...
  const totalColSpan = selectedBodega > 0 ? 8 : 9;
  const footerGroup = (
    <ColumnGroup>
      <Row>
        <Column
  ...
  return (
    <div>
      ...
      <div className="p-col-12">
        ...
        >
          ...
          <Column
            ...
            bodyStyle={{ width: "2rem", textAlign: "center" }}
          />
        </DataTable>
      </div>
      ...
      <ProductSearchDialog
        ...
        initialSelectedProducts={selectedProducts} // Pasa los productos seleccionados
      />
      ...
      >
        <div className="confirmation-content">
          ...
          {product && (
            <span>
              ¿Desea borrar el producto <b>{product.nombreproducto}?</b>
            </span>
          )}
        </div>
      </Dialog>
