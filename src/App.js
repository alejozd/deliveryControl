...
function App() {
  ...
  const apiUrlEstado = `${config.apiUrl}/Datasnap/rest/TServerMethods1/ConsultarEstado`;
  ...
  const consultarAutorizacion = async () => {
    try {
      const response = await axios.get(apiUrlEstado);
      if (response.data.estado) {
        setEstadoAutorizacion(response.data.estado);
